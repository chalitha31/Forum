...
?>
        <div class="commented-unit">
            <div class="commented-channel-pic-holder">
                <?php
                $proComntRs  = Database::search("SELECT * FROM `profile_pic` WHERE `email` = '" . $comntuser["email"] . "'");
                $picComntrow = $proComntRs->num_rows;
                if ($picComntrow == "1") {
                    $picComntData = $proComntRs->fetch_assoc();
                ?>
                    <img onclick='profileview("<?php echo $comntuser["email"] ?>");' class="commented-channel-pic" src="images/profile_pic/<?php echo $picComntData["image"] ?>" alt="channel-pic" />
                <?php
                } else {
                ?>
                    <img onclick='profileview("<?php echo $comntuser["email"] ?>");' class="commented-channel-pic" src="images/profile_pic/user.svg" alt="channel-pic" />
                <?php
                }
                ?>
            ...
            <div class="commented-discription">
                ...
                <div class="comment-name-unit">
                    <p onclick='profileview("<?php echo $comntuser["email"] ?>");' class="commented-person"><?php echo $comntuser["username"] ?></p>
